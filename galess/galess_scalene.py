...
def Lensed_Point_LF(M_int, dPdMu, LF_func, zs):
    ...
    if hasattr(M_int, '__len__'): res = [np.trapz(dPdMu(mu)*LF_func(M+5/2*np.log10(mu), zs), mu) for M in M_int]
    else: res = np.trapz(dPdMu(mu)*LF_func(M_int+5/2*np.log10(mu), zs), mu)
    return np.array(res)

def Lensing_Bias(M_int, dPdMu, LF_func, zs):
    if hasattr(M_int, '__len__'): res = [Lensed_Point_LF(M, dPdMu, LF_func, zs)/LF_func(M, zs) for M in M_int]
    else: res = Lensed_Point_LF(M_int, dPdMu, LF_func, zs) / LF_func(M_int, zs)
    return np.array(res)
...
def Fraction_2nd_image_above_Mlim(M_int, M_lim, LF_func, zs, Phi_vel_disp = Phi_vel_disp_Mason):
    tau = Tau(zs, Phi_vel_disp)
    dPdMu = dPdMu_Point_SIS
    if hasattr(M_int, '__len__'): res = [integral.quad(Lensed_Point_LF_integrand, mu_lim_second_image(M, M_lim), 1e3, args=(M, dPdMu, LF_func, zs))[0]/LF_func(M, zs) for M in M_int]
    else: res = integral.quad(Lensed_Point_LF_integrand, mu_lim_second_image(M_int, M_lim), 1e3, args=(M_int, dPdMu, LF_func, zs))[0]/LF_func(M_int, zs)
    return np.nan_to_num(tau * np.array(res) / (tau * Lensing_Bias(1, M_int, LF_func, zs) + (1 - tau)))    
    
def Fraction_1st_image_arc(mu_arc, M_int, LF_func, zs, Phi_vel_disp = Phi_vel_disp_Mason):
    tau = Tau(zs, Phi_vel_disp)
    dPdMu = dPdMu_Point_SIS
    if hasattr(M_int, '__len__'):
        res = [integral.quad(Lensed_Point_LF_integrand, mu_arc, 1e3, args=(M, dPdMu, LF_func, zs))[0]/LF_func(M, zs) for M in M_int]
    else: res = integral.quad(Lensed_Point_LF_integrand, mu_arc, 1e3, args=(M_int, dPdMu, LF_func, zs))[0]/LF_func(M_int, zs)
    return np.nan_to_num(tau * np.array(res) / (tau * Lensing_Bias(1, M_int, LF_func, zs) + (1 - tau)))
...
def Lensing_Bias_SIE(img_N, M_int, LF_func, zs):
    if hasattr(M_int, '__len__'): res = [Lensed_Point_LF_SIE(img_N, M, LF_func, zs) / LF_func(M, zs) for M in M_int]
    else: res = Lensed_Point_LF_SIE(img_N, M_int, LF_func, zs) / LF_func(M_int, zs)
    return res
...
def Fraction_Nth_image_above_Mlim_SIE(img_N, M_int, M_lim, LF_func, zs, Phi_vel_disp = Phi_vel_disp_Mason):
    if  (img_N==1): dP_dmu_SIE, w = __dP_dmu_SIE1, __w1
    elif(img_N==2): dP_dmu_SIE, w = __dP_dmu_SIE2, __w2
    elif(img_N==3): dP_dmu_SIE, w = __dP_dmu_SIE3, __w3
    elif(img_N==4): dP_dmu_SIE, w = __dP_dmu_SIE4, __w4
    else: return 0
    if hasattr(M_int, '__len__'): res = [Lensed_Point_LF_integral_SIE(1, mu_lim_Nth_image_SIE(img_N, M, M_lim, zs), M, LF_func, zs) / Lensed_Point_LF_SIE(1, M, LF_func, zs) for M in M_int]
    else: res = Lensed_Point_LF_integral_SIE(1, mu_lim_Nth_image_SIE(img_N, M_int, M_lim, zs), M_int, LF_func, zs) / Lensed_Point_LF_SIE(1, M_int, LF_func, zs)
    return np.nan_to_num(np.array(res))
    
def Fraction_1st_image_arc_SIE(mu_arc, M_int, LF_func, zs, Phi_vel_disp = Phi_vel_disp_Mason):
    if hasattr(M_int, '__len__'): res = [Lensed_Point_LF_integral_SIE(1, mu_arc, M, LF_func, zs) / Lensed_Point_LF_SIE(1, M, LF_func, zs) for M in M_int]
    else: res = Lensed_Point_LF_integral_SIE(1, mu_arc, M_int, LF_func, zs)/ Lensed_Point_LF_SIE(1, M_int, LF_func, zs)
    return np.nan_to_num(np.array(res))
...
def get_prob_lensed_bckgnd(sigma, zl, zs, M_array, dzs=0.5, SIE_FLAG=False):   
    ...
    prob_lens = [Calculate_prob_lens(sigma, zl, zs, M, dM, dzs, SIE_FLAG) for M in M_array]
    return np.nan_to_num(np.array(prob_lens))
...
def calculate_num_lenses_and_prob(sigma_array, zl_array, zs_array, M_array_UV, app_magn_limit, survey_area_sq_degrees,
                                  ...
                                  Phi_vel_disp = Phi_vel_disp_Mason, LENS_LIGHT_FLAG = False, SIE_FLAG = True):
                ...
                for isg, sigma in enumerate(sigma_array):
                    ...
                    for izl, zl in enumerate(zl_array):
                        ...
                        if((zs1>zl) and (Theta_E(sigma, zl, zs1)>seeing_trsh*seeing_arcsec)):
                            ...
                            #We approximate the selection of the arc strect OR the second image above M_lim with the max (eval at each mag bin)
                            SNR_1img_z1 = 1 #Signal_to_noise_ratio(m_array-2.5*np.log10(3), Source_size_arcsec(M_array_UV, zs1), sky_bckgnd_m_per_arcsec_sq, zero_point_m, exp_time_sec, num_exposures = num_exposures)>=SNR
                            SNR_2img_z1 = 1 #Signal_to_noise_ratio(m_array, Source_size_arcsec(M_array_UV, zs1),  sky_bckgnd_m_per_arcsec_sq, zero_point_m, exp_time_sec, num_exposures = num_exposures)>=SNR
                            SNR_1img_z2 = 1 #Signal_to_noise_ratio(m_array-2.5*np.log10(3), Source_size_arcsec(M_array_UV, zs2), sky_bckgnd_m_per_arcsec_sq, zero_point_m, exp_time_sec, num_exposures = num_exposures)>=SNR
                            SNR_2img_z2 = 1 #Signal_to_noise_ratio(m_array, Source_size_arcsec(M_array_UV, zs2),  sky_bckgnd_m_per_arcsec_sq, zero_point_m, exp_time_sec, num_exposures = num_exposures)>=SNR
...
sigma_array = np.arange(100 , 400 , 30 )
zl_array    = np.arange(0.  , 1.5 , 0.1)
zs_array    = np.arange(0.  , 2.8 , 0.3)

matrix_noLL, Theta_E_noLL, prob_noLL = calculate_num_lenses_and_prob(
