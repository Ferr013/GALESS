...
def Lens_cone_volume_diff(z, area_sq_degree, dz=0.5):  #works only in a flat universe
    c_sp = 299792.458                                  #km/s
    Hz = cosmo.H(z).value                              #km s^-1 Mpc^-1
    area_sterad = area_sq_degree*1/(57.2958**2)        #sterad 
    return np.power(cosmo.angular_diameter_distance(z).value*(1+z),2)*(c_sp/Hz)*area_sterad*dz
...
def Fraction_lensed(M_int, dPdMu, LF_func, zs):
    ...
    if hasattr(M_int, '__len__'):
        res = np.zeros(0)
        for M in M_int: 
            res = np.append(res,  tau*Lensing_Bias(M, dPdMu, LF_func, zs)/(tau*Lensing_Bias(M, dPdMu, LF_func, zs)+(1-tau)))
        return res
...
############################################################################################################################################
### From Lenstronomy Data Util (https://lenstronomy.readthedocs.io/en/latest/_modules/lenstronomy/Util/data_util.html#flux_noise) ##########
############################################################################################################################################
...
############################################################################################################################################
### [END] From Lenstronomy Data Util (https://lenstronomy.readthedocs.io/en/latest/_modules/lenstronomy/Util/data_util.html#flux_noise) ####
############################################################################################################################################

def Signal_to_noise_ratio(source_magnitude, exp_time_sec, sky_bckgnd_m_per_arcsec_sq, zero_point_m, pixel_arcsec, readout_noise = 0, num_exposures = 1):
    ...
    #cps_sky_varnc = flux_noise(bkg_noise_cps, exp_time_sec)
    #mag_sky_varnc = (cps2magnitude(bkg_noise_cps-cps_sky_varnc, zero_point_m)-cps2magnitude(bkg_noise_cps+cps_sky_varnc, zero_point_m))/2
    #bkg_noise_mag = cps2magnitude(bkg_noise_cps, zero_point_m)
    return magnitude2cps(source_magnitude, zero_point_m)/bkg_noise_cps
...
######### SIE LENS FUNCTIONS #######################################################################################################################
weights = np.loadtxt('SIE_dPdmu/weights_dP_dmu_SIE.txt')
_internal_dP_dmu_SIE1, _internal_w1 = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_1.txt'), weights[0]
_internal_dP_dmu_SIE2, _internal_w2 = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_2.txt'), weights[1]
_internal_dP_dmu_SIE3, _internal_w3 = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_3.txt'), weights[2]
_internal_dP_dmu_SIE4, _internal_w4 = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_4.txt'), weights[3]

def Lensed_Point_LF_SIE(img_N, M_int, LF_func, zs):
    if  (img_N==1): dP_dmu_SIE, w = _internal_dP_dmu_SIE1, _internal_w1
    elif(img_N==2): dP_dmu_SIE, w = _internal_dP_dmu_SIE2, _internal_w2
    elif(img_N==3): dP_dmu_SIE, w = _internal_dP_dmu_SIE3, _internal_w3
    elif(img_N==4): dP_dmu_SIE, w = _internal_dP_dmu_SIE4, _internal_w4
    else: return 0
...
def mu_lim_Nth_image_SIE(img_N, M, M_lim, zs): 
    ...
    if(img_N==2):
        dP_dmu_A = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_1.txt')
        dP_dmu_B = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_2.txt')
    elif(img_N==3):
        dP_dmu_A = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_1_3reg.txt')
        dP_dmu_B = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_3_3reg.txt')
    elif(img_N==4):
        dP_dmu_A = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_1_4reg.txt')
        dP_dmu_B = np.loadtxt('SIE_dPdmu/dP_dmu_SIE_4.txt')
    else: return 0
...
def Check_R_from_sigma_FP(sigma, zl, zs, M_array_UV, obs_photo_band, Ls_M0 = -21, Ls_gamma = 0.25, Ls_R0 = 1): 
    ...
    arr_logRe, frac1st, frac2nd = np.linspace(-0.5,1.5,10), 0, 0
    ...
    R_e_Ma_arc = (np.power(10,(M_array_UV-Ls_M0)*(-0.4*Ls_gamma))*Ls_R0)/(cosmo.angular_diameter_distance(zs).value*1e3)*206265 #Get Source SB from L-size relation with NO lensing in kpc -> arcsec
    SB_iLF = M_array_UV+5*np.log10(cosmo.luminosity_distance(zs).value*1e5)+2.5*np.log10(np.pi*np.power(R_e_Ma_arc,2)) #mag arcsec^-2
...
def calculate_num_lenses_and_prob(sigma_array, zl_array, zs_array, M_array_UV, app_magn_limit, survey_area_sq_degrees,
                                  seeing_arcsec, SNR, exp_time_sec, sky_bckgnd_m_per_arcsec_sq, zero_point_m, pixel_arcsec,  
                                  photo_band, mag_cut = None, arc_mu_threshold = 3, seeing_trsh = 1.5, readout_noise = 0, num_exposures = 1, 
    ...
    # get limit magnitude given signal-to-noise ratio
    SNR_lim_mag  = limit_magnitude_given_SNR(SNR, exp_time_sec, sky_bckgnd_m_per_arcsec_sq, zero_point_m, pixel_arcsec, readout_noise, num_exposures)
    ...
    for izs, zs in enumerate(zs_array):
        ...
        M_lim   = M_lim_b-K_correction_from_UV(zs, photo_band, M_lim_b)
        #if the magnitude related to the target SNR ratio is brighter thatn the limit magnitude, use that as a limit for the cumulative prob.
        #M_lim  = M_lim if (app_magn_limit-obs_band_to_intr_UV_corr) < (SNR_lim_mag-obs_band_to_intr_UV_corr) else (SNR_lim_mag-obs_band_to_intr_UV_corr)
        ...
        for isg, sigma in enumerate(sigma_array):
            ...
            for izl, zl in enumerate(zl_array):
                ...
                if((zs>zl) and (Theta_E(sigma, zl, zs)>seeing_trsh*seeing_arcsec)): #Einstein radius is > seeing_trsh*seeing (https://arxiv.org/pdf/2301.03670.pdf)
                    ...
                    Ngal_tensor[izs][isg][izl][:] = prob_lens*np.max(np.vstack((frac_arc*weight_1img, frac_2nd_img*weight_2img)), axis=0)*Phi_vel_disp_Mason(sigma, zl)*_dsg*Lens_cone_volume_diff(zl, survey_area_sq_degrees, dz=_dzl)
...
def get_N_and_P_projections(N_gal_matrix, sigma_array, zl_array, zs_array):
